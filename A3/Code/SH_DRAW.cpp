...
void SingleHandler_DRAW::do_GATHER_DRAWCALLS(Events::Event *pEvt)
{
    ...
    if (pMeshCaller->m_performBoundingVolumeCulling)
    {
		...
		for (int iInst = 0; iInst < pMeshCaller->m_instances.m_size; ++iInst)
		{
			...
			SceneNode *curSN = hParentSN.getObject<SceneNode>();
            ...
            if (curSN)
            {
				...
				for (Vector3 vertex : pMeshCaller->m_AABBPoints)
				{
				...
				for (int i = 0; i < 4; ++i)
				{
					...
					Matrix4x4 m = worldMatrix * pMeshCaller->m_AABB[i];
					DebugRenderer::Instance()->createAABBLineMesh(true, m, NULL, 0, 0);
				}
            }
        }
    }
