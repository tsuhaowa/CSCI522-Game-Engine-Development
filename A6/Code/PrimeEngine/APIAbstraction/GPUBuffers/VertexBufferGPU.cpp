...
//OGL is still using old way of forwarding all source buffers to low level helper implementation
	...
	m_pBufferSetInfo = &VertexBufferGPUManager::Instance()->m_vertexBufferInfos[PEVertexFormatLayout_DetailedMesh_B0__P0f3_B1__TC0f_B2__N0f3_B3__T0f3];
#else
	PositionBufferCPU res(*m_pContext, m_arena);
	res.m_values.reset((vb.m_values.m_size / 3) * (3 + 2 + 2 + 3 + 3));

	// create 2nd uv set
	TexCoordBufferCPU tcb2(*m_pContext, m_arena);
	tcb2.m_values.reset(tcb.m_values.m_size);	
	...
	for (PrimitiveTypes::UInt32 iv = 0; iv < vb.m_values.m_size / 3; iv++)
	{
		...
		x = vb.m_values[curInex]; y = vb.m_values[curInex + 1]; z = vb.m_values[curInex + 2];
		u = tcb.m_values[itcval++]; v = tcb.m_values[itcval++];
		tcb2.m_values.add(y); tcb2.m_values.add(y);
		u2 = y; v2 = y;
		nx = nb.m_values[curInex]; ny = nb.m_values[curInex + 1]; nz = nb.m_values[curInex + 2];
		tx = tb.m_values[curInex]; ty = tb.m_values[curInex + 1]; tz = tb.m_values[curInex + 2];
	...
	m_pBufferSetInfo = &VertexBufferGPUManager::Instance()->m_vertexBufferInfos[PEVertexFormatLayout_DetailedMesh_B0__P0f3_TC0f2_TC7f2_N0f3_T0f3];
