    ...
    if (pMeshCaller->m_performBoundingVolumeCulling)
    {
		...
		for (int iInst = 0; iInst < pMeshCaller->m_instances.m_size; ++iInst)
		{
			...
			else 
            {
				...
				// update bounding box 8 vertices
				for (int vIndex = 0; vIndex < 8; ++vIndex)
				{
					Vector3 vertex = pPhyManager->m_boundingBoxVertex[vIndex];
					Vector3 vertexTransformed = worldMatrix * vertex;
					int insideCounter = 0;
					...
					{
						pInst->m_culledOut = false;
						// not break to update the pPhyManager->m_boundingBoxVertexAfterTransform[vIndex]
						// break;
					}
				}

				// update pPhyManager AferTransform datas
				pPhyManager->buildBoundingVolumeAfterTransform(worldMatrix);

				// out of frustum, no need to draw
				...
				for (int i = 0; i < 4; ++i)
				{
					...
					DebugRenderer::Instance()->createAABBLineMesh(true, 
						pPhyManager->m_boundingBoxAfterTransform[i], NULL, 0, 0);
				}
            }
        }
    }
