...
void ClientGameObjectManagerAddon::addDefaultComponents()
{
	...
	PE_REGISTER_EVENT_HANDLER(Event_CREATE_NAZICAR, ClientGameObjectManagerAddon::do_CREATE_NAZICAR);
...
void ClientGameObjectManagerAddon::do_CREATE_NAZICAR(PE::Events::Event *pEvt)
{
	PEINFO("GameObjectManagerAddon::do_CREATE_NAZICAR()\n");

	assert(pEvt->isInstanceOf<Event_CREATE_NAZICAR>());

	Event_CREATE_NAZICAR *pTrueEvent = (Event_CREATE_NAZICAR*)(pEvt);

	PE::Handle hNaziCar("NaziCar", sizeof(NaziCar));
	NaziCar *pNaziCar = new(hNaziCar) NaziCar(*m_pContext, m_arena, hNaziCar, pTrueEvent);
	pNaziCar->addDefaultComponents();

	addComponent(hNaziCar);
}
