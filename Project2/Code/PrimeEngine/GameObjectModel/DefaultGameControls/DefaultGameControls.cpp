...
#include "CharacterControl/NaziCar/NaziCar.h"
...
void DefaultGameControls::handleKeyboardDebugInputEvents(Event *pEvt)
{
	...
	{
		int cameraIndex = CameraManager::getCameraIndex();
		if (cameraIndex == 0)
		{
			Handle h("EVENT", sizeof(Event_ROTATE_CAMERA));
			Event_ROTATE_CAMERA *rotateCameraEvt = new(h) Event_ROTATE_CAMERA;
		...
		{
			SceneNode *pMainSN = CameraManager::getSelectedSceneNode(cameraIndex);
			pMainSN->m_physicsComponent->m_forceLR = -pMainSN->m_physicsComponent->m_torque;
		}
	}
	else if (Event_KEY_DOWN_HELD::GetClassId() == pEvt->getClassId())
	{
		...
		{
			SceneNode *pMainSN = CameraManager::getSelectedSceneNode(cameraIndex);
			pMainSN->m_physicsComponent->m_forceFB = -pMainSN->m_physicsComponent->m_horsepower;
		}
	}
	...
	else if (Event_KEY_F1_HELD::GetClassId() == pEvt->getClassId())
	{
		CameraManager::setCameraIndex(0);
	}
	else if (Event_KEY_F2_HELD::GetClassId() == pEvt->getClassId())
	{
		CameraManager::setCameraIndex(1);
	}
	else if (Event_KEY_F3_HELD::GetClassId() == pEvt->getClassId())
	{
