...
void PhysicsComponent::init()
{
	...
	if (strcmp(m_meshFileName, SOLDIER_MESH_NAME) == 0)
		init_Soldier();
	else
		init_StaticMesh();
}
...
void PhysicsComponent::buildBoundingVolumeAfterTransform(const Matrix4x4 &worldMatrix)
{
	...
	if (strcmp(m_meshFileName, NAZICAR_MESH_NAME) == 0 && m_id != -1)
	{
		for (int i = 0; i < 4; ++i)
		{
			m_carDirAfterTransform[i] = worldMatrix * m_carDir[i] - m_pMainSN->m_base.getPos();
			m_carDirAfterTransform[i].normalize();
		}
	}
}
...
void PhysicsComponent::addToCameraList()
{
	m_carDir[0] = Vector3(0, 0, -1);
	m_carDir[1] = Vector3(0, 0, 1);
	m_carDir[2] = Vector3(1, 0, 0);
	m_carDir[3] = Vector3(-1, 0, 0);

	CameraManager::addSelectedSceneNode(m_pMainSN);
}
