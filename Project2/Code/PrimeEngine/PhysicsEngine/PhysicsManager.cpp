...
#define PHYSICS_COMPONENT_SIZE 1600
...
void PhysicsManager::do_PRE_PHYSICS_UPDATE()
{
	...
	for (PhysicsComponent &phyC : m_phyComponents)
	{
		...
		if (phyC.m_id == -1 && pMainSN->m_base == phyC.m_prevMainBase)
			continue;  // cull out unchanged object

		
		m_updatePhyComponents.push_back(&phyC);
		
		if (strcmp(phyC.m_meshFileName, SOLDIER_MESH_NAME) == 0)
		{
		...
		else  
		{
			...
			phyC.buildBoundingVolumeAfterTransform(pMainSN->m_base);
		}
...
void PhysicsManager::do_SIMULATION()
{
	...
	{
		if (strcmp(curPhyC->m_meshFileName, SOLDIER_MESH_NAME) == 0) {
			do_SoldierSimulation(curPhyC);
		}
		else if (strcmp(curPhyC->m_meshFileName, NAZICAR_MESH_NAME) == 0) {
			do_NazicarSimulation(curPhyC);
		}
	}
}

void PhysicsManager::do_SoldierSimulation(PhysicsComponent *curPhyC)
{
	float speed = 1.4f;
	...
	if (checkCollisionToALL(curPhyC))
	{
		...
		if (curPhyC->m_collisionCount == 1 && !(curPhyC->m_prevMainBase.getPos().getY() == curPhyC->m_postMainBase.getPos().getY()))
		{
			...
			// rescale the dir
			curPhyC->m_postMainBase.m16[7] = curPhyC->m_curStandPlane->m_boundingBoxPlanes[3].getFloorHeight();  // 7 is y pos 
...
void PhysicsManager::do_NazicarSimulation(PhysicsComponent *curPhyC)
{
	float speed = 1.4f;
	float allowedDisp = speed * m_frameTime;
	Vector3 dir;
...
bool PhysicsManager::checkCollisionToALL(PhysicsComponent *curPhyC)
{
	...
	for (int i=0; i<m_phyComponents.size(); ++i)
	{
		...
		if (checkCollisionTwoPhysicsComponent(curPhyC, t_phyC))
		{
			...
			if (strcmp(t_phyC->m_meshFileName, COBBLEPLANE_MESH_NAME) == 0)
			{
				curPhyC->m_curStandPlane = t_phyC;  // top plane of standing floor
				continue;
			}
...
bool PhysicsManager::checkCollisionPlane(PhysicsComponent *curPhyC, Plane &p)
{
	...
	for (int i = 0; i < 8; ++i) {
		...
		if (d_centerToP <= d_projection) return true;
	}
	return false;
}