...
void CameraSceneNode::do_CALCULATE_TRANSFORMATIONS(Events::Event *pEvt)
{
	...
	// use a wider frustum plane to do culling
	PrimitiveTypes::Float32 verticalFov2 = 0.36f * PrimitiveTypes::Constants::c_Pi_F32;
	if (aspect < 1.0f)
	{
		//ios portrait view
		static PrimitiveTypes::Float32 factor = 0.5f;
	...
	Matrix4x4 mvp = m_viewToProjectedTransform2 * m_worldToViewTransform;
	mvp = mvp.transpose();
