...
void DefaultAnimationSM::do_PRE_RENDER_needsRC(PE::Events::Event *pEvt)
{
	...
	if (g_iDebugBoneSegment < 0 && !g_debugSkinning)
	{
		for (int i = 0; i < m_modelSpacePalette.m_size; ++i)
		{
			...
			float x = m.getPos().getX();
			float y = m.getPos().getY();
			float z = m.getPos().getZ();

			PhysicsComponent::setExtremeValue(x, y, z, minX, maxX, minY, maxY, minZ, maxZ);
		}

		PhysicsComponent *phyC = pSN->getFirstParentByTypePtr<SceneNode>()->getFirstParentByTypePtr<SceneNode>()->m_physicsComponent;
		if (phyC)
		{
			// build bounding box
			phyC->buildBoundingVolume(minX, maxX, minY, maxY, minZ, maxZ);
			phyC->buildBoundingVolumeAfterTransform(Matrix4x4());
