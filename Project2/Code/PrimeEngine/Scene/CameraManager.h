...
struct CameraManager : public Component
{
	...
	static void addSelectedSceneNode(SceneNode *pSN) { m_cameraSwitch.push_back(pSN); }
	static void setCameraIndex(int n) { m_cameraIndex = n; }
	static SceneNode* getSelectedSceneNode(int n) { return n < m_cameraSwitch.size() ? m_cameraSwitch[n] : nullptr; }
	static int getCameraIndex() { return m_cameraIndex; }
	static std::vector<SceneNode*> m_cameraSwitch;
