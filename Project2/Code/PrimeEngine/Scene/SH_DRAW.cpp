...
#include "PrimeEngine/PhysicsEngine/PhysicsComponent.h"
...
void SingleHandler_DRAW::do_GATHER_DRAWCALLS(Events::Event *pEvt)
{
    ...
    if (pMeshCaller->m_performBoundingVolumeCulling)
    {
        ...
        for (int iInst = 0; iInst < pMeshCaller->m_instances.m_size; ++iInst)
        {
			...
			// get this meshInstance SceneNode
			SceneNode *curSN = pInst->getFirstParentByTypePtr<SceneNode>();
			SkeletonInstance *pSI = NULL;

			if (curSN == NULL)  // may be soldier's mesh, complex structure
			{
				...
				if (pSI)
				{
					...
					curSN = pSN->getFirstParentByTypePtr<SceneNode>();
					pInst->m_culledOut = false;
					++pMeshCaller->m_numVisibleInstances;
				}
			}
			else if(curSN->m_physicsComponent)
			{
				// get bounding box info from PhysicsManager of this MeshInstance
				PhysicsComponent *phyC = curSN->m_physicsComponent;

				pInst->m_culledOut = true;
				Matrix4x4 worldMatrix = curSN->m_worldTransform;
				
				// check if box is inside the frustum planes
				for (int vIndex = 0; vIndex < 8; ++vIndex)
				{
					Vector3 vertexTransformed = phyC->m_boundingBoxVertexAfterTransform[vIndex];
					int insideCounter = 0;
					for (Plane plane : pcam->m_frustumPlanes)
					{
						if (plane.isInsidePlane(vertexTransformed))
							++insideCounter;
						else 
							break;
					}

					if (insideCounter == 6)  // one vertex is inside the frustum
					{
						pInst->m_culledOut = false;
						break;
					}
				...
				// if out of frustum, no need to draw
				if (pInst->m_culledOut) continue;

				// else, draw them and AABB box
				++pMeshCaller->m_numVisibleInstances;

				// draw AABB
				if (DebugRenderer::drawBoundingBox)
				{
					for (int i = 0; i < 4; ++i)
					{
						// draw AABB boxes by 4 matrices from 4 vertices
						DebugRenderer::Instance()->createBoundingBoxLineMesh(true,
							phyC->m_boundingBoxAfterTransform[i], NULL, 0, 0);
					}
				}
			}
        }
