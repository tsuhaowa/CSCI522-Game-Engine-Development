...
// 			}
    ...
    int maxX = 25; // maybe need more to get framerate lower
    ...
    if (spawnALotOfMeshes)
    {
        for (int ix = 0; ix < maxX; ++ix)
        {
            for (int iy = 0; iy < 10; ++iy)
            {
				...
				PhysicsManager::add(pMainSN, "imrod.x_imrodmesh_mesh.mesha");
	...
	bool spawnProjectTerrain = true;

	int cobblePlaneMaxX = 10;
	int cobblePlaneMaxZ = 10;

	if (spawnProjectTerrain)
	{
		for (int ix = 0; ix < cobblePlaneMaxX; ++ix)
		{
			for (int iz = 0; iz < cobblePlaneMaxZ; ++iz)
			{
				PE::Handle hSN("SCENE_NODE", sizeof(SceneNode));
				SceneNode *pMainSN = new(hSN) SceneNode(*m_pContext, m_arena, hSN);
				pMainSN->addDefaultComponents();

				pMainSN->m_base.setPos(Vector3(-ix * 25.0f, 0, 12.5f + iz * 25.0f));

				PE::Handle hCobblePlaneInst = PE::Handle("MeshInstance", sizeof(MeshInstance));
				MeshInstance *pCobblePlaneInst = new(hCobblePlaneInst) MeshInstance(*m_pContext, m_arena, hCobblePlaneInst);

				pCobblePlaneInst->addDefaultComponents();
				pCobblePlaneInst->initFromFile("cobbleplane.x_pplaneshape1_mesh.mesha", "Default", m_pContext->m_gameThreadThreadOwnershipMask);


				pMainSN->addComponent(hCobblePlaneInst);

				RootSceneNode::Instance()->addComponent(hSN);

				PhysicsManager::add(pMainSN, "cobbleplane.x_pplaneshape1_mesh.mesha");
				pMainSN->m_physicsComponent = PhysicsManager::getLastPhysicsComponentAddress();
				pMainSN->m_physicsComponent->m_frictionCoef = 0.004;



				PE::Handle hSN2("SCENE_NODE", sizeof(SceneNode));
				SceneNode *pMainSN2 = new(hSN2) SceneNode(*m_pContext, m_arena, hSN2);
				pMainSN2->addDefaultComponents();

				pMainSN2->m_base.setPos(Vector3(-ix * 25.0f, 0, -12.5f - iz * 25.0f));

				PE::Handle hCobblePlaneInst2 = PE::Handle("MeshInstance", sizeof(MeshInstance));
				MeshInstance *pCobblePlaneInst2 = new(hCobblePlaneInst2) MeshInstance(*m_pContext, m_arena, hCobblePlaneInst2);

				pCobblePlaneInst2->addDefaultComponents();
				pCobblePlaneInst2->initFromFile("cobbleplane.x_pplaneshape1_mesh.mesha", "Default", m_pContext->m_gameThreadThreadOwnershipMask);

				pMainSN2->addComponent(hCobblePlaneInst2);

				RootSceneNode::Instance()->addComponent(hSN2);

				PhysicsManager::add(pMainSN2, "cobbleplane.x_pplaneshape1_mesh.mesha");
				pMainSN2->m_physicsComponent = PhysicsManager::getLastPhysicsComponentAddress();
				pMainSN2->m_physicsComponent->m_frictionCoef = 0.001;
			}
		}
	}
